...
class Displaystore_Gallery_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getDownloadFileUrl($url, $disposition) {
        return Mage::getUrl('gallery/download/download', array('_query' => array('d' => $disposition, 'file' => $url)));
    }
    ...
    public function getFilesHtml($url, $title, $id = null, $showTitle = false, $disposition = 0, $size = false) {
        ...
        if ($disposition) {
            if ($ext == 'jpg' || $ext == 'jpeg' || $ext == 'gif' || $ext == 'png' || $ext == 'bmp') {
                $image = $mediaUrl . $url;
                $onclick = "return galleryPopup.open({url:this.rel, title: '" . str_replace(' ', '_', $title) . "'});";
