...
class Displaystore_Gallery_DownloadController extends Mage_Core_Controller_Front_Action {
    ...
    protected function _processDownload($file,$disposition=0) {
        $_helper = Mage::helper('gallery/download');
        $resource = $_helper->getFilePath(Mage::getBaseDir('media'), $file);
        $resourceType = Displaystore_Gallery_Helper_Download::LINK_TYPE_FILE;
    ...
    public function downloadAction() {
        ...
        if ($fileName) {
            try {
                ...
                $this->_getSession()->addError(Mage::helper('gallery')->__('Sorry, there was an error getting requested content.'));
            }
        }
